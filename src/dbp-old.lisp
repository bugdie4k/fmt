...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defclass+ prefix-section (token))
  (defclass+ message-section (token))
  ...
  (defmacro kw^defkeywords (&rest definitions-by-type)
    (flet ((%accessor (slot-kw)
      ...
      `(progn ,@(mappend
                 (lambda (definition)
                   (destructuring-bind (keyword-type parse-lexem-fn
                                                     &rest entries)
                       definition
                     (let ((accessor (%accessor keyword-type)))
                       (cons
                        `(setf (,accessor *keywords*)
                               (cons (make-hash-table) ,parse-lexem-fn))
                        (mapcar
                         (lambda (entry)
                           (destructuring-bind (name &rest value) entry
                             (let ((value (if (cdr value)
                                              ...
                                              (car value))))
                               `(setf
                                 (gethash ,name (car (,accessor *keywords*)))
                                 ,value))))
                         entries)))))
                 definitions-by-type))))
  ...
  (defun parse-sd/prefix (lexem-next-args)
    (values (make-instance 'prefix-section) lexem-next-args))

  (defun parse-sd/message (lexem-next-args)
    (values (make-instance 'message-section) lexem-next-args))
  ...
  (defun parse-section-designator-lexem (lexem lexem-next-args)
    (parse-2valued-keyword (kw^section-designators-alist)
                           lexem lexem-next-args))
  ...
  (kw^defkeywords
   (:section-designators #'parse-section-designator-lexem
    (:prefix  "p>" #'parse-sd/prefix)
    (:message "m>" #'parse-sd/message))
   (:markup #'parse-markup-lexem
    (:literally           "l"   #'parse-kw-mrkup/literally)
    (:delimiter           "d"   #'parse-kw-mrkup/delimiter)
    (:newline             "nl"  (curry #'parse-kw-mrkup/newline nil))
    (:conditional-newline "cnl" (curry #'parse-kw-mrkup/newline t)))
   (:returns #'parse-return-lexem
    (:return       "r="  (curry #'parse-kw-return nil))
    (:print-return "pr=" (curry #'parse-kw-return t)))
   (:options #'parse-option-lexem
  ...
  (defun parse-symbol (sym-name args)
    (let ((lexem-next-args (rest args)))
      (multiple-value-bind (token rest-args)
        ...
        (if token
            (values token rest-args)
            (values (make-instance 'regular-token
                                   :form (first args)
                                   :generated-symbol nil)
                    (rest-args))))))
  ...
  (defun lexer (args &optional tokens)
    (if (not body)
        tokens
        (aif (symbol-name? (first body))
             (multiple-value-bind (token rest-args)
                 (parse-symbol it args)
               (lexer rest-args
                      (cons token tokens)))
             (lexer (rest args)
                    (cons (make-instance 'regular-token
                                         :form (first args)
                                         :generated-symbol (gensym "REGULAR"))
                          tokens)))))
